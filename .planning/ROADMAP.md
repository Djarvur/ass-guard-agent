...
- **Phase 14 (new):** ground truth in-repo — `research/ECOSYSTEM-AUDIT.md` (§3.3 borrow ranking, §4.4 SNAP/CACHE/ECON/TOOLCON, §5-1 pi cross-validation), `research/IDEA-LANDSCAPE.md` §borrow-list (🧲-1 checkpoints/undo with cline shadow-git / opencode `/undo` / LangGraph checkpointer as design references), SEED-004's tags, and for EARLY-04 the pi repo itself (`packages/ai/src/api/anthropic-messages.ts` + `transform-messages.ts` + its test suite as the behavioral spec). Plan directly; no other external research expected.

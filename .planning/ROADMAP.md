...
**Milestone:** v1.1 ACP Early Adoption (Phases 8, 9, 12, 14, 13; numbering continues from v1.0's Phase 7 — never reset)
**Requirements mapped:** 30/30 v1.1 ✓
**Milestone complete 2026-08-21:** all phases complete (8, 9, 12, 13, 14 — Phases 10/11 parked to v1.2), 34/34 plans; the completion bar (operator begins daily ACP use) was met at the adoption line 2026-08-20; verified gates: `mise ci` green + eval-gate flagship green k=1 (manager-certified 2026-08-20; Phase 13 closed on 13-VERIFICATION 3/3 PASS)

v1.1 completes the product. Phase 8 closed the kickoff loop (`/opsx:explore → propose → apply → archive` chained by the engine with zero manual continues, verified against the real openspec binary) — v1.0's major known gap. **Operator re-order 2026-08-18 (early adoption):** "I want to start use this agent ASAP" — everything not necessary to START daily ACP use is postponed behind an explicit **adoption line**. First step executed: the other-agents analysis (`research/ECOSYSTEM-AUDIT.md` + `research/IDEA-LANDSCAPE.md` via SEED-004) is dispositioned into **Phase 14: Adoption Readiness** — the operator took the FULL six-item YES set ("делаем все 1-6"): workspace undo/checkpoints (the no-confirmation safety backstop — both analyses' #1), compaction policy verified from the Phase-9 pin before designing, a cache-hit-behavior probe (daily cost drift detectable), pi↔shaper cross-validation (north-star hardening), token economics (light-tier subagents + truncation), and the uniform tool contract (is_error/per-tool timeouts/retry-transient/flags). The NO items stay recorded, not dropped (steering queue → v1.2 Telegram replan tag; real sandbox → 12-06's existing tag; LSP/MEM/TREE/OTEL/HEART/marketplace → v1.2+ unchanged). The milestone completes when the operator begins daily use; the post-adoption continuation (12-03/07/08, 13) executes while the agent is in use. **Operator re-scope 2026-08-16 + phase split:** Telegram (ex-Phase 10) and the dsh profile (ex-Phase 11) are lowered to the v1.2 planning pool in favor of two new phases — **Phase 12: Product Functional Completeness** (every built-in catalog tool executes for real — 9 still return "no implementation yet" — result forms capture-pinned, the behavioral-eval regression net, plugin-install discovery) and **Phase 13: OpenSpec Workflow Completion** (the expanded command matrix runs E2E with zero-continue chaining and eval coverage). Phase 9 stays next (it *is* product-surface work: serve-path audit + parity re-capture). Scope discipline after the re-scope: **zero new dependencies in v1.1** (go-telegram/bot, zstd, and the `internal/runtime` refactor move to v1.2 with their phases), and the generalized v1.0 lesson holds as a cross-phase invariant — **no feature closes with stub-only evidence; every external surface carries a real-binary/live-service gate**.
...
- ✅ **v1.1 ACP Early Adoption** — Phases 8, 9, 12, 14, 13 (5/5 complete; renamed "Kickoff & Peers" → "ACP Completion" → "Product Completion" at the 2026-08-16 re-scope + split, → "ACP Early Adoption" at the 2026-08-18 adoption re-order; COMPLETE 2026-08-21 — the completion bar was met at the adoption line 2026-08-20 and the post-adoption continuation finished in use)
...
- [x] **Phase 13: OpenSpec Workflow Completion** - the expanded OpenSpec command matrix (`new / continue / ff / verify / bulk-archive / onboard`) runs E2E with zero-continue chaining and eval coverage *(the 2026-08-16 split's workflow half; executes after Phase 12 — post-adoption continuation. Completed 2026-08-21 — verification 3/3 PASS; matrix E2E 12 legs, zero-continue chains re-proven live, 12 per-command eval suites green)*
...
| 13 | OpenSpec Workflow Completion | 5/5 | Completed 2026-08-21 — verification 3/3 PASS; matrix E2E 12 legs, zero-continue chains re-proven live, 12 per-command eval suites green | 2026-08-21 |
...
**Plans:** 5/5 plans complete (verified 2026-08-21; planned 2026-08-19 — PATTERNS ground truth + a planning-time live probe sharpened the profile bootstrap: the pinned v1.5.0 has NO named "expanded" preset — the switch is `profile: "custom"` + a `workflows` array in the global config, verified end-to-end in an isolated HOME with all 11 workflows installing; 13-00 added 2026-08-20 as the wave-0 gap closure)

Plans:
**Wave 0** *(added 2026-08-20 — the ask-suspension × chaining gap closure; manager Rule-4 route 1, FLAGGED for retroactive operator confirmation)*

- [x] 13-00-PLAN.md — engine-visible ask resume: session settle seam + engine ask-wait + serve park + harness wait-mirror; exit = mise eval-gate flagship FIRST GREEN through asks [OS-02] — COMPLETE 2026-08-20 (flagship green, artifact eval-20260820-161520-k1.json; both Phase-13 blockers closed; ACP-08's deferred exit evidence discharged)

**Wave 1**

- [x] 13-01-PLAN.md — expanded-profile matrix E2E: the shared global-config guard in internal/evalharness (byte-exact save/restore, absent-key case), newOpsxMatrixRunner, all 6 happy legs with captured closings (D-03 pass 1) + all 6 fixable legs (D-01's 12-leg matrix; onboard idempotent re-run per D-08), the 6 [command_mutability] rows [OS-01] — THE phase tracer — COMPLETE 2026-08-20 (12-leg matrix green — 6 happy + 6 fixable with deterministic probed triggers, 12 captures committed under testdata/opsx-e2e-matrix/; operator config byte-guarded; Task 1's gate re-verified GREEN, artifact eval-20260820-163220-k1.json)
**Wave 2** *(13-02 blocked on 13-01's captures)*

- [x] 13-02-PLAN.md — zero-continue chaining pass 2: harvest-derived seed rows incl. the D-07 verify→fix handoff + terminal shields, per-chain zero-continue legs with audit-trail evidence (EngineDecision + CommandProvenance), structural-safety re-pin, D-06 flake policy [OS-02] — COMPLETE 2026-08-20 (post-verify-handoff + post-new-continue-handoff seeded; both chain legs green with audit-trail evidence; ff/bulk/onboard termini dispositioned in-table)
...
- [x] 13-03-PLAN.md — the D-02/D-05 advisory: question-shaped-ending classifier + Decide's unmatched cell (Action stays Nothing), post-turn session/update note from the in-hand emit (the timing hazard designed around), per-session+class dedupe on the ACP wrapper (engine stays stateless — the new-state disposition) + mandatory-conditional dead-end evidence: advisory lines proven on matrix transcripts, gated onboard-tutorial re-proof when the awareness lever fires [OS-02] — COMPLETE 2026-08-20 (classifier + unmatched-cell advisory battery green; the awareness lever honestly NOT applied — corpus shows AskUserQuestion usage, recorded; dead-end scan folded into the onboard leg)
- [x] 13-04-PLAN.md — the six per-command eval suites (happy + fixable each, D-04) extending 12-08's net on the same re-run gate, with the expanded-profile guard wired into the scenario path + exact failure attribution [OS-03] — COMPLETE 2026-08-20 (12 scenarios + flagship; FULL batch pass@1 TRUE 12/12, artifact eval-20260820-202612-k1.json; TestEvalSuite_Matrix_Gated gate surface; mise ci unchanged)

### Phase 14: Adoption Readiness (Analysis Dispositions)

**Goal:** As the operator about to daily-drive ass-guard from an ACP editor, I want the one unrecoverable-risk backstop (workspace undo) and the two cheap evidence checks (compaction behavior, cache discipline) that the cross-agent analyses flagged, so that early adoption starts on a controllable, cost-predictable agent — not a leap of faith.
**Mode:** mvp
**Depends on:** Nothing hard (builds on shipped v1.0 + Phases 8–9; the Phase-9 pinned capture is EARLY-02/03's ground truth). Sequenced after 12-01/12-02 so the two already-checker-passed plans start with zero planning latency; this phase's own planning may overlap their execution.
**Added at the 2026-08-18 adoption re-order** — the operator's "process the other-agents analysis first" directive, dispositioned, then EXPANDED same day by the operator's "делаем все 1-6" (the full pre-adoption candidate set). Triage of ECOSYSTEM-AUDIT §3.3 borrow ranking + §4.4 small items and IDEA-LANDSCAPE §borrow-list (SEED-004's four gaps) against "worth implementing BEFORE daily use begins": **YES (all six)** — checkpoints/undo (both analyses' #1; the field's universal answer to a no-confirmation agent: cline shadow-git, opencode `/undo`, gemini-cli, LangGraph checkpointer), compaction verify-first (cheap, decisive), cache-hit probe (daily use spends real money), pi↔shaper cross-validation (hardens the wire chokepoint against pi's 43k-line behavioral spec), token economics (light-tier subagent routing + tool-output truncation), uniform tool contract (is_error, per-tool timeouts, retry-only-transient, concurrency/destructive flags). **NO, recorded** — steering/input queue (→ v1.2 Telegram replan tag, IDEA-LANDSCAPE gap 4), real sandbox flag (→ 12-06's existing SEED-004 tag), LSP/MEM/TREE/OTEL/HEART/marketplace lifecycle (→ v1.2+ unchanged).
**2026-08-19 new-data disposition (operator priority ruling: P1 agent usable ASAP / P2 everything important before use / P3 functionality + autotests):** the research amendment `cb3ab50` (OpenClaude + occ deep-dives; borrows #13-15) is dispositioned against the adoption line — **the line does NOT move**: nothing in the new analysis is a daily-use dead end or an unrecoverable risk. **#15 surface census** (occ `env.mjs` ~100 env vars, 25 tool names, 6 permission-mode names, 4 MCP transports) enters as a FREE verification input — EARLY-06's contract inventory cross-checks it, and 12-06/12-07's catalog-completeness tests gain it as a coverage manifest (zero new requirement, zero scope growth). **#13 nightly-parity gate** (occ ADR-001): a cheap target-version drift-warning slice is a Phase 14 planning input for EARLY-03 (the parity run already records `zcode_version` provenance from 09-03 — surface a loud warning when the installed zcode differs from the pinned capture); the full nightly CI automation lands with 12-08's EVAL gate (post-adoption), further automation → v1.2. **#14 scheduler outcome store** (occ cost-cascade feedback loop) → the v1.2 pool (needs accumulated usage data to matter; the pre-adoption economics slice is already EARLY-05).
**Requirements:** EARLY-01, EARLY-02, EARLY-03, EARLY-04, EARLY-05, EARLY-06
**Success Criteria** (what must be TRUE):

  1. A bad mutating turn is recoverable — ass-guard snapshots workspace file state at turn boundaries into a shadow-git store under the ass-guard root (external git; never touches the user's repo index/remotes/HEAD; no daemon, no port), and `ass-guard checkpoint list|restore` works from the terminal, proven by a live scratch-project test: a real turn mutates the workspace → restore → workspace byte-identical, the user's `.git` untouched (EARLY-01)
  2. The compaction question is answered from evidence — the Phase-9 pinned capture is analyzed for zcode auto-compact / context-eviction / `cache_control` breakpoint behavior; the finding is committed as a decision artifact, and either "mimicry already delivers it" is documented and closed or a scoped implementation requirement is issued to the post-adoption queue (EARLY-02)
  3. Cache-hit behavior is observable — the parity harness carries a probe asserting dynamic merges (skills, MCP tools) respect stable→volatile ordering and captured `cache_control` placement against the pin, wired into the existing parity run so daily-use cache/cost drift is detectable, not just structural divergence (EARLY-03)
  4. The shaper is cross-validated against pi's wire layer — a committed audit diffs `internal/shaper` against pi's `anthropic-messages.ts` + `transform-messages.ts` (cache_control placement, thinking-config mapping, header merge order, compat-flag catalog), and every divergence found is dispositioned: fixed, justified in-repo, or routed with rationale (EARLY-04)
  5. Token economics are explicit — subagent turns route through the scheduler `light` tier by default (configurable), and a tool-output truncation policy bounds oversized results in the captured form before they enter the projected window, both proven by tests (EARLY-05)
...
- [x] 14-03-PLAN.md — pi↔shaper cross-validation: shallow re-clone at a pinned commit (gitignored, never vendored), the five-dimension audit (cache_control, thinking, headers, compat flags, message mapping), every divergence dispositioned, minimal capture-faithful fixes [EARLY-04]
...
- [x] 14-05-PLAN.md — token economics: subagent model routing through the existing `tiers.light` config surface (same-provider override on the per-dispatch profile copy; cross-provider degrades loudly + routed) + tool-output truncation at the append chokepoint (128 KiB cap, corpus-scale results untouched, marker flagged corpus-absent → 12-05) [EARLY-05]
- [x] 14-06-PLAN.md — uniform tool contract: catalog-wide per-tool timeout backstop at DispatchBatch (inner deadlines win, siblings isolated), corpus-grounded is_error inventory + fixes, retry-only-transient pins, concurrency/destructive flags feeding dispatch (D-21 floor untouched), and the occ surface-census cross-check as INPUT ONLY (pinned clone, names-only fixture) [EARLY-06]

## Dependency Chains

- **Adoption line re-order (2026-08-18):** execution runs 12-01 + 12-02 → **Phase 14** (new — the other-agents-analysis dispositions, all six operator-confirmed: undo/checkpoints, compaction verify, cache probe, pi↔shaper cross-validation, token economics, tool contract) → 12-05 → 12-04 → 12-06 → **ADOPTION LINE** (the operator begins daily ACP use) → 12-03 → 12-08 → 12-07 → 13. Everything above the line is either a daily-use dead-end remover (12-01/02/04/06 + 12-05's forms) or an adoption-risk backstop (Phase 14); nothing below the line blocks starting daily use. The parked Phase-10 WIP (`47f10b4` on the phase branch, untrusted) is NOT a v1.1 dependency — Phase 12's plans reference wiring sites by symbol and were written against both layouts.
...
- **Phase 13 (new):** ground truth already in-repo — the openspec v1.5.0 installed-binary surface the Phase-8 adapter probe pinned, the Phase-8 chaining/provenance decision record (STATE.md), and the 08-06 E2E harness as template. Plan directly; no research phase expected. Post-adoption continuation.
- **Phase 14 (new):** ground truth in-repo — `research/ECOSYSTEM-AUDIT.md` (§3.3 borrow ranking, §4.4 SNAP/CACHE/ECON/TOOLCON, §5-1 pi cross-validation, §1.1 occ census), `research/IDEA-LANDSCAPE.md` §borrow-list (🧲-1 checkpoints/undo with cline shadow-git / opencode `/undo` / LangGraph checkpointer as design references), SEED-004's tags, and for EARLY-04 the pi repo itself (`packages/ai/src/api/anthropic-messages.ts` + `transform-messages.ts` + its test suite as the behavioral spec). Plan directly; no other external research expected. **2026-08-19 planning inputs (borrow #13/#15 dispositions):** EARLY-06's inventory cross-checks the occ surface census (~100 `CLAUDE_CODE_*` env vars / 25 tool names / 6 permission-mode names / 4 MCP transports — a free coverage manifest for toolcat/profile/config; caveat from §1.1: names are real, behavioral depth varies); EARLY-03's parity-run wiring considers the cheap target-version drift warning (installed zcode version vs the pinned capture's — loud, not blocking).
...
**Execution order:** 8 → 9 → [12-01, 12-02] → 14 → [12-05, 12-04, 12-06] → **ADOPTION LINE** → [12-03, 12-08, 12-07] → 13 (2026-08-18 early-adoption re-order; strict chain, no parallel execution across phases — Phase 14's PLANNING may overlap 12-01/02 EXECUTION). Phases 10–11 moved to the v1.2 pool 2026-08-16.
...
| 14. Adoption Readiness (Analysis Dispositions) | v1.1 | 6/6 | Complete (verification pass 39/39; all four human items resolved in one operator session; live rollback E2E PASS) | 2026-08-19 |
| 13. OpenSpec Workflow Completion | v1.1 | 5/5 | Complete (verification 3/3 PASS — both zero-continue chain legs re-run green by the verifier; 12 per-command eval suites green k=1) | 2026-08-21 |
...
---
*Roadmap created: 2026-08-14 · Re-scoped: 2026-08-16 (operator — phases 10/11 lowered to the v1.2 pool; Phase 12 + Phase 13 added via re-scope + split; milestone renamed v1.1 Product Completion) · Re-ordered: 2026-08-18 (operator — early adoption: Phase 14 added from the other-agents-analysis dispositions; Phase 12 split around the adoption line; milestone renamed v1.1 ACP Early Adoption)*

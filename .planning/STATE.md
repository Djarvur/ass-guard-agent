...
status: planning
last_updated: "2026-08-14T15:55:14.672Z"
...
| 7 — Multi-Provider Config & Credentials | **COMPLETE (2/2 plans)** | PCFG-01..04 + PROV-01 done. 07-01 (schema + resolver + factory): `ProviderConfig.{APIKey,APIKeyEnv}`, `ResolveCredential` (flag > env > config, `${VAR}`), `ProviderFactory.Build` per declared provider (wire-proven base_url + key), lazy `noCredentialProvider` (D-07), embedded default + seed `api_key_env: ZAI_API_KEY` byte-identical. 07-02 (wiring): all three construction sites (acp serve makeProvider, main.go tracer, parity.go) delegate to the factory via `loadSchedulingFactory`/`setupProviderFactory`; acp serve keeps zero direct `NewAnthropicProvider` calls; D-07 startup uncredentialed warnings + SC3 0600-perm warning on stderr; SC4 backward-compat + SC1 editor-zero-env proven by tests; `(*ProviderFactory).Endpoint` accessor added for the LOG-01 tracer capturer (flagged cross-plan edit). Phase gate GREEN: `mise ci` exits 0 (vet + lint 0 issues + build + test -race all packages). |

## Decisions Log

- **UAT deferral — Phase 4 kickoff gap (2026-08-14, operator):** `/gsd-verify-work 4` (MVP-mode UAT) FAILED at user-flow step 1: the OpenSpec kickoff requires claude-code-compatible slash-command invocation (`/opsx:explore` etc., installed by `openspec init --tools claude` into `.claude/commands/opsx/`), but `internal/ecosys` discovery is unwired (zero non-test importers — no ACP/session invocation surface) and Phase 4's subprocess adapter models the binary as stage driver with a command set mismatching openspec v1.5.0's real surface. Full diagnosis in `04-UAT.md` → Gaps. Operator decision: fix DEFERRED to next-milestone scoping (wire ecosys into session/ACP + `/namespace:command` expansion + adapter reconciliation, together with the deepseek-harness profile candidate above). v1.0 ships verified-with-known-gap; 11 UAT checks deferred until the entry surface works.
- **Direction clarification — mimicry targets (2026-08-14, operator):** opencode is NOT a mimicry target (it appears in research docs as landscape/survey context only; the claude-code-compat zcode profile remains profile #1). Operator context: daily model access runs through an opencode subscription serving MiniMax + DeepSeek models. Tentative future direction: profile #2 candidate = `deepseek-ai/deepseek-harness` (DeepSeek's official agent harness "dsh" — plugin-based TypeScript, MIT, ships AGENTS.md/CLAUDE.md conventions; repo verified to exist 2026-08-14) for DeepSeek-model turns — rationale: the profile should match the harness the target model actually behaves in. Feasibility: Phase 7's two-shape provider layer (anthropic/openai + configurable base_url + ProviderFactory) is the structural prerequisite; dsh being open-source means ground-truth capture can come from source + its own logs rather than MITM-only. Not scheduled — to be scoped at next `/gsd-new-milestone`.
- **Plan 07-02 (2026-08-14):** Phase 7 Plan 2 COMPLETE (3/3 TDD tasks, 5 commits, integration wave). Factory wiring (D-08): `cmd/ass-guard/provider_factory.go` adds `loadSchedulingFactory(workDir, apiKeyFlag, stderr)` (overlay `.ass-guard/scheduling.yaml` on the embedded default, build `scheduler.ProviderFactory`, emit D-07 `WarnUncredentialed` to stderr) + `setupProviderFactory` (graceful degrade to embedded default on operator-config errors, T-07-08; heavy tier resolved via the validated scheduler resolver, first-declared-provider fallback). All three former hardcoded sites replaced: acp_serve makeProvider → `factory.Build`, main.go tracer → `tracerProvider` (reconstructs the AnthropicProvider with factory-RESOLVED base_url+key to attach the LOG-01 RequestCapturer via the new `(*ProviderFactory).Endpoint` cross-plan accessor), parity → factory-built heavy-tier provider. Startup warnings: uncredentialed providers (D-07) + SC3 0600-perm hygiene (`warnLooseConfigPerm`). Proofs: SC4 `TestBackwardCompat_ZAIEnvOnly` (embedded default + $ZAI_API_KEY → Source=env) + SC1 `TestEditorZeroEnv_LiteralInConfig` (literal api_key → Source=config, zero env). `grep 'NewAnthropicProvider(shaper.New())' cmd/ass-guard/` → zero matches (e2e fixture rewired too). Coverage reconciled: 71 v1 reqs (Phase 7 = 4 PCFG; PROV-01 counted under Phase 1). `mise ci` gate green. Phase 7 COMPLETE. Next: milestone ship (`/gsd:verify-work 07` + `/gsd:complete-milestone`).
...
- [Phase 7]: Plan 07-02 (2026-08-14): the LOG-01 tracer's RequestCapturer seam is handled by a new `(*scheduler.ProviderFactory).Endpoint(providerName) (baseURL, key, ok)` accessor (flagged cross-plan edit to 07-01's factory.go): `tracerProvider` reconstructs the AnthropicProvider with factory-RESOLVED base_url+key + `WithAnthropicRequestCapture` — never hardcoded defaults (D-08). Rejected alternatives: a capturer setter on the adapter or a Build variadic (both widen the factory/adapter API for one caller).
- [Phase 7]: Plan 07-02 (2026-08-14): operator-config load failures degrade to the embedded-default factory + a stderr log at ALL THREE wiring sites via the shared `setupProviderFactory` (T-07-08, mirroring the engine/learning degradation pattern) — only a failure of the embedded default itself (impossible in practice) fails the process. `warnLooseConfigPerm` is acp-serve-only (the editor-spawned path where the operator's file matters most).

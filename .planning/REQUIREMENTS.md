...
- [ ] **DSH-04**: An operator selects `--profile dsh`, `scheduling.yaml` carries a credentialed DeepSeek provider entry, `profile check dsh` detects drift against the per-profile capture, and a sanitized dsh seed ships in the embedded defaults

...
severity: major
resolution: "Ruling (b) implemented by gap-closure plan 19-06 (armed carve-out + content-sensitive overflow-retry battery + WR-03 riders) and 19-07 (armed override precedence over pre-user markers). Re-proven LIVE by the auto-UAT harness 2026-09-10: overflow rejection → forced compact → retry 515664B → 95605B carrying the FORCED marker's summary, turn completes end_turn (uat-harness-run-20260910/requests.jsonl seq 7-9)."
...
severity: major
resolution: "Root cause was a stale binary (G-19-2 diagnosis). Automated live retest 2026-09-10 (operator-approved stand-in, spikes/19-compaction-live-uat): HEAD build driven as a fake editor over ACP stdio with a mock Anthropic provider — threshold 50 live-set via set_config_option, first + second compactions fire (markers on disk, stderr notes, summary-seeded continuations), restore 80 persists, overflow retry survives. PASS 20/20. The invisibility caveat stands and stays in Deferred Follow-Ups (Phase 20 /status vehicle)."
...
- gap_id: G-19-1
  ...
  status: resolved
  resolved_by: 19-06-SUMMARY.md (gap_ids: [G-19-1]) + 19-07-SUMMARY.md (armed-override precedence)
...
- gap_id: G-19-2
  ...
  status: resolved
  resolved_by: auto-UAT retest (spikes/19-compaction-live-uat, PASS 20/20 — 19-HUMAN-UAT.md test 1)
  resolved_at: 2026-09-10
  ...
  resolution: "Fresh binary installed 2026-09-07 (v0.0.0-20260907153836-a8f63f31b1b0, engine string verified present). Retest executed 2026-09-10 as an automated live harness (operator ruling: no manual leg): threshold 50 → two compactions with markers/notes/summary seeds, restore 80 persisted, overflow backstop proven. Evidence: uat-harness-run-20260910/."

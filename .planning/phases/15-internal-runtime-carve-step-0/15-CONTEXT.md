...
- **D-13:** Sub-package split NOW, 3-way (operator choice over single package): `internal/runtime` (runner core), `internal/runtime/enginebridge` (7 adapters + engine setup helpers), `internal/runtime/cron` (cron_wiring.go's 9 methods re-homed + scheduler loop). acpserve composes all three.
  - **AMENDED 2026-08-26 (operator sign-off during plan-check revision):** the cron leg is deferred — Go forbids declaring methods outside their receiver's package and core↔cron calls are bidirectional (4 of 9 cron methods called from runner core; runAutomationTurn → runOneTurn), so a separate `internal/runtime/cron` package today forces non-verbatim body rewrites against the verbatim-move bar. Resolution: `cron_wiring.go` relocates byte-verbatim to `internal/runtime/cron_wiring.go` (package runtime); no cron sub-package is created in Phase 15. The 2-way shape (runtime + enginebridge) ships now; Phase 25 introduces the interface-based cron seam when it can be done as design work rather than relocation collateral.
...
- **D-15:** cron methods ride with the runtime family (not left at wiring layer) — they are methods on the runner today; they land in internal/runtime/cron.
  - **AMENDED 2026-08-26 (operator sign-off during plan-check revision):** they land beside their receiver in package runtime (`internal/runtime/cron_wiring.go`), per the D-13 amendment above.

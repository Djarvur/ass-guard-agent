...
### CR-01: `session/delete` racing an in-flight `session/load` resurrects a deleted session (D-07 break, silent post-delete data loss)
...
### WR-01: `Runner.ResumeSession` skips the per-session turn mutex (17-REVIEW CR-02 discipline not extended to the load path)
...
### WR-03: In-process re-load after `session/close` reuses a half-reaped runner session (MCP host dead, TranscriptWriter cancelled, forwarder stopped)
...
### WR-04: `--continue` + `--resume` both typed: code lets `--continue` win, doc (and intent) say `--resume` wins
